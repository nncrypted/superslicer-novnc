...
if [[ ! -e "$baseDir/latestReleaseInfo.json" ]]; then
  ...
  curl -SsL https://api.github.com/repos/supermerill/SuperSlicer/releases/163509270 > $baseDir/latestReleaseInfo.json
...
if [[ $# -gt 1 ]]; then
  ...
  if [[ ! -e "$baseDir/releases.json" ]]; then
    curl -SsL https://api.github.com/repos/supermerill/SuperSlicer/releases/163509270 > $baseDir/releases.json
  fi
